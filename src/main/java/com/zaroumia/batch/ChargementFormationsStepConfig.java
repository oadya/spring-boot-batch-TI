...
public class ChargementFormationsStepConfig {
	...
	public StaxEventItemReader<Formation> formationItemReader(
		...
		return new StaxEventItemReaderBuilder<Formation>().name("formationItemReader")
				.resource(inputFile)
				.addFragmentRootElements("formation") // quel est la balise à extraire
				.unmarshaller(formationMarshaller()) // pour convertir la balise vers l'objet de domaine formation
				.build();
	}
